...
function getAgeColor(createdAt) {
  ...
  if (ms < 3600000)    return "#f87171"; // < 1h  — very new, high risk
  if (ms < 86400000)   return "#fb923c"; // < 1d  — new
  if (ms < 604800000)  return "#f59e0b"; // < 7d  — recent
  if (ms < 2592000000) return "#94a3b8"; // < 30d — established
...
// ─── Guide Section helpers (module-level to avoid React unmount/remount anti-pattern) ───
const GuideS = ({ children, color = "#818cf8" }) => (
  <span style={{ color, fontWeight: 700 }}>{children}</span>
);
const GuideKbd = ({ children }) => (
...
const GuideSection = ({ icon, title, color, children }) => (
...
const GuideRow = ({ icon, label, desc }) => (
...
export default function App() {
  ...
  const applyFilters = (list) => {
    ...
    const maxAgeMs = maxAgeDays !== "" ? parseFloat(maxAgeDays) * 86400000 : null;
    const now = Date.now();
    return list
      .filter((t) => {
    ...
    <div className="app-bg" style={{ minHeight: "100vh", padding: "32px 24px", maxWidth: 1120, margin: "0 auto" }}>
      ...
      {activeSection === "guide" && (
          <div className="animate-in" style={{ maxWidth: 760, margin: "0 auto" }}>
            ...
            <GuideSection icon="🔥" title="Discover" color="#f59e0b">
              <GuideRow icon="📋" label="Trending tokens" desc="Auto-fetched from GeckoTerminal every 60 seconds. Filter by chain with the buttons at the top right." />
              <GuideRow icon="🔍" label="Token search" desc="Type to filter by symbol, name, or contract address. Press / to jump straight to the search box." />
              <GuideRow icon="📌" label="Pin a token" desc="Click the 🔖 bookmark icon on any card. Pinned tokens appear at the top and keep refreshing even if they fall off trending." />
              <GuideRow icon="🔔" label="Price alerts" desc="Click the 🔔 bell on any card. Set a target price (above or below). A toast fires the moment the price crosses. Manage all alerts from the bell icon in the header." />
              <GuideRow icon="📈" label="Mini chart" desc="Click 📈 to expand a 1H/12H/1D price or market-cap chart built from live OHLCV data." />
              <GuideRow icon="👥" label="Holder analysis" desc="Click 👥 (Solana only) to see the top 20 holder accounts, their share of supply, and a concentration bar. Whale-heavy tokens (top 10 > 60%) are flagged red." />
              <GuideRow icon="🫧" label="BubbleMaps" desc="Click 🫧 to open the token on BubbleMaps for cluster analysis." />
              <GuideRow icon="DS" label="DexScreener" desc="Click DS to open the token's DexScreener page in a new tab." />
              <GuideRow icon="📝" label="Token notes" desc="Pinned tokens have a notes textarea at the bottom. Write entry targets, risk notes, anything. Auto-saved." />
              <GuideRow icon="➕" label="Track a custom CA" desc="Press N (or click + Track CA) to pin any token by contract address directly, even before it's trending." />
              <GuideTip>Pin your watchlist first, then check the 👜 badge on each card — it shows how many of your tracked wallets hold that token.</GuideTip>
            </GuideSection>

            <GuideSection icon="👜" title="Wallets" color="#4ade80">
              <GuideRow icon="➕" label="Add a wallet" desc="Press N, paste any Solana or EVM address. Holdings load automatically with live USD values." />
              <GuideRow icon="📥" label="Import wallets" desc="Click ↑ Import to load a JSON file. Accepts arrays of addresses or {address, label} objects — including exports from this app." />
              <GuideRow icon="📤" label="Export" desc="↓ CSV exports all token positions across all wallets. ↓ JSON exports your wallet list for backup or sharing." />
              <GuideRow icon="🔍" label="Cross-wallet search" desc="Type in the search bar above the wallet list to filter by token symbol, mint address, or wallet label/address. All matching cards auto-expand." />
              <GuideRow icon="📝" label="Wallet notes" desc="Expand any wallet card — there's a freeform notes area at the bottom. Auto-saved per wallet address." />
              <GuideRow icon="✏️" label="Wallet labels" desc="Click the wallet label (or 'Add label') to rename it inline. Press Enter or click away to save." />
              <GuideRow icon="↕" label="Sort & view" desc="Sort wallets by value or by added order. Toggle between card view and compact list view with the ⊞/≡ buttons." />
              <GuideRow icon="📊" label="Portfolio summary" desc="Appears above your wallet list. Shows total value, 24h P&L, top movers, allocation bar, sparkline history, and a per-wallet breakdown." />
              <GuideTip>Use '/' to jump to the search box and type a token symbol to instantly find which of your wallets hold it — all matching cards expand automatically.</GuideTip>
            </GuideSection>

            <GuideSection icon="📡" title="Monitor" color="#f59e0b">
              <GuideRow icon="🔭" label="What it does" desc="Scans the on-chain transactions of your pinned tokens and discovers wallets that traded them recently." />
              <GuideRow icon="▶️" label="Scan Now" desc="Click Scan Now to trigger a manual scan. The monitor looks for fresh wallets (new to trading) and dormant wallets (suddenly active)." />
              <GuideRow icon="⚙️" label="Settings" desc="Configure which chains to scan, how many transactions to analyze, and alert thresholds." />
              <GuideRow icon="🚨" label="Alerts" desc="Discovered wallets appear as alerts — you can inspect them, track them directly, or dismiss." />
              <GuideTip>Combine Monitor with Wallets: when Monitor finds an interesting whale wallet, click Track to add it — then see its full holdings in the Wallets tab.</GuideTip>
            </GuideSection>

            <GuideSection icon="🔔" title="Price Alerts" color="#f59e0b">
              <GuideRow icon="1️⃣" label="Set an alert" desc="Click 🔔 on any token card → choose 'above' or 'below' → enter your target price → click Set." />
              <GuideRow icon="2️⃣" label="Alert fires" desc="When the token price crosses your target, a toast notification appears immediately. The alert is marked ✓ triggered." />
              <GuideRow icon="3️⃣" label="Manage alerts" desc="Click the 🔔 bell in the top-right of the header to open the alerts panel. Per-alert dismiss, 'Clear triggered', or 'Clear all'." />
              <GuideTip>Alerts are checked every time price data refreshes (~60 seconds). For best results, pin the token so its price data stays current even off trending.</GuideTip>
            </GuideSection>

            <GuideSection icon="⌨️" title="Keyboard Shortcuts" color="#818cf8">
              {[
                ...
                <div key={key} style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  <GuideKbd>{key}</GuideKbd>
                  <span style={{ fontSize: 12, color: "#64748b" }}>{desc}</span>
                </div>
              ))}
            </GuideSection>

            <GuideSection icon="💡" title="Tips & Tricks" color="#34d399">
              <GuideTip>
                <GuideS color="#4ade80">Best workflow:</GuideS> Pin the tokens you're researching → open the 👥 holder panel to check concentration → track the top holder wallets → use the portfolio summary to see your total exposure.
              </GuideTip>
              <GuideTip>
                <GuideS color="#4ade80">Research notes:</GuideS> Pin a token, scroll to the bottom of its card, and write notes directly there. Great for entry price targets, risk flags, or news context. Notes survive page reloads.
              </GuideTip>
              <GuideTip>
                <GuideS color="#4ade80">Backup wallets:</GuideS> Click ↓ JSON in the Wallets tab to download your wallet list. To restore later, use ↑ Import. Duplicate wallets are silently skipped.
              </GuideTip>
              <GuideTip>
                <GuideS color="#4ade80">Cross-wallet search:</GuideS> Want to know which of your wallets hold BONK? Press <GuideKbd>/</GuideKbd> in the Wallets tab and type "BONK". All matching wallets expand instantly.
              </GuideTip>
              <GuideTip>
                <GuideS color="#4ade80">Price alert strategy:</GuideS> Pin a token, set a "below" alert at your stop-loss price and an "above" alert at your take-profit target. The toast fires whether the tab is active or not (as long as the page is open).
              </GuideTip>
            </GuideSection>
